...
app.post('/reserve', (req, res) => {
  ...
  db.query('SELECT RoomCost FROM rooms WHERE ID = ?', [RID], (err, result) => {
    ...
    let roomCost = result[0].RoomCost;

    // If no services are selected, handle it accordingly
    if (!selectedServices || selectedServices.length === 0) {
      const servicecost = 0;
      let total = parseInt(roomCost) + servicecost;
      console.log(total);
      const state = 0;

      const checkin_date = new Date(req.body.checkin); // Example: '2024-06-22'
      const checkout_date = new Date(req.body.checkout); // Example: '2024-06-24'
 
      const oneDay = 24 * 60 * 60 * 1000; // Hours*Minutes*Seconds*Milliseconds
      const diffDays = (Math.round(Math.abs((checkin_date - checkout_date) / oneDay)) + 1);
     ...
     console.log("total final ",total);
      ...
      db.query('SELECT SUM(ServiceCost) AS ServiceCost FROM services WHERE ID IN (?)', [serviceIDs], (err, result1) => {
        ...
        let servicecost = result1[0].ServiceCost;
        console.log(servicecost);
        let total = parseInt(roomCost) + parseInt(servicecost);
        console.log("totla + roomCost"+total);
        const state = 0;
...
app.get('/ownerhome/:UID', (req, res) => {
  const UID = req.params.UID;
  // هنا يمكنك إضافة الكود لمعالجة الطلب وإرجاع الصفحة المناسبة
  res.render('ownerhome.ejs', { UID: UID });
});
